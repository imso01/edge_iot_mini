...
int main(int argc, char *argv[])
{
		...
		CLIENT_INFO client_info[MAX_CLNT] = {{0,-1,"","1","PASSWD"}, \
				...
				{0,-1,"","KJW_SQL","PASSWD"},  {0,-1,"","KJW_BT","PASSWD"}, \
				{0,-1,"","KJW_STM32","PASSWD"},  {0,-1,"","KSH_LIN","PASSWD"}, \
				{0,-1,"","KJW_AND","PASSWD"},  {0,-1,"","KJW_ARD","PASSWD"}, \
				{0,-1,"","HM_CON","PASSWD"}};
...
void * clnt_connection(void *arg)
{
	...
	while(1)
	{
		...
		// // 📌 센서 데이터인 경우에만 클라이언트에게 전달
		// if (pArray[1] != NULL && strcmp(pArray[1], "SENSOR") == 0) {
		// 	msg_info.fd = client_info->fd;
		// 	msg_info.from = client_info->id;
		// 	msg_info.to = "KJW_SQL";  // DB에 저장하는 클라이언트
		// 	sprintf(to_msg, "[%s]%s@%s@%s@%s@%s", msg_info.from, pArray[1], pArray[2], pArray[3], pArray[4], pArray[5]);
		// 	msg_info.msg = to_msg;
		// 	msg_info.len = strlen(to_msg);

		// 	sprintf(strBuff, "Forwarding SENSOR data: [%s -> %s] %s", msg_info.from, msg_info.to, msg_info.msg);
		// 	log_file(strBuff);
		// 	send_msg(&msg_info, first_client_info);  // 데이터 전달
		// }
	}
